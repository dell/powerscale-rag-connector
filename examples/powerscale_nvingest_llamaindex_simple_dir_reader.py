#!/usr/bin/env python3
"""
PowerScale NvIngest RAG Vectorstore Example — LlamaIndex SimpleDirectoryReader variant

This example uses PowerScaleSimpleDirectoryReader with NvIngest v2 and ElasticsearchStore.
...
Components used:
  - PowerScaleSimpleDirectoryReader (LlamaIndex) for change detection
  - llama-index-vector-stores-elasticsearch for the vectorstore
  - llama-index-embeddings-nvidia for NIM embeddings

PowerScaleSimpleDirectoryReader wraps LlamaIndex's SimpleDirectoryReader with MetadataIQ
change detection. It yields LlamaIndex Document objects where:
  ...
  - text                     : file content read by SimpleDirectoryReader (ignored here;
                               NvIngest handles extraction for consistent chunking)
...
Configuration (.env):
  ...
  INPUT_DIR              - PowerScale folder path to monitor (must start with /ifs)
...
def delete_by_lin(vector_store: ElasticsearchStore, lin: int) -> int:
    try:
        deleted_count = asyncio.run(_delete_by_lin_async(vector_store, lin))
...
def add_chunks_to_vectorstore(
    ...
    try:
        ...
        for chunk, embedding in zip(chunks, embeddings):
            ...
            nodes.append(TextNode(text=chunk.get("text", ""), embedding=embedding, metadata=metadata))

        vector_store.add(nodes)
        logger.info("Added %d chunks to vectorstore for lin=%d", len(nodes), lin)
        return len(nodes)
    except Exception as e:
...
async def _close_store(store: ElasticsearchStore) -> None:
    try:
...
def process_changed_files() -> None:
    ...
    loader = PowerScaleSimpleDirectoryReader(
        ...
        app_name="powerscale_nvingest_rag_simpledir",
        app_version=1,
    )
    ...
    for document in loader.lazy_load_data():
        ...
        added = add_chunks_to_vectorstore(vector_store, embed_model, chunks, lin, str(filepath), change_types)
        if added > 0:
            success_count += 1
        else:
            error_count += 1
...
if __name__ == "__main__":
    try:
        ...
        logger.info("Starting PowerScale NvIngest RAG processing (SimpleDirectoryReader)")
        logger.info("PowerScale path: %s", INPUT_DIR)
