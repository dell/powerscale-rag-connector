...
- **Checkpoint auto-commits when generators are fully consumed**: all loaders, readers, and `get_directory_changes()` now save the checkpoint automatically when the scan completes. `PowerScaleSimpleDirectoryReader` defers the checkpoint write until after parsing succeeds.
