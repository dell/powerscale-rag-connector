...
- **Public `save_checkpoint()` methods on all loaders and readers**: `PowerScaleUnstructuredLoader`, `PowerScaleUnstructuredReader`, and `PowerScaleSimpleDirectoryReader` now expose a thin `save_checkpoint()` method that delegates to their internal helper/path loader. This gives the README examples a consistent API and lets callers explicitly advance the checkpoint if they break out of a generator early.
...
- **`get_directory_changes(save_checkpoint=False)` default**: the previous default (`True`) caused every caller to pass `save_checkpoint=False` explicitly. The default is now `False`, making the opt-in intent clear: callers must explicitly request an automatic checkpoint write when they do not need to perform additional downstream work.
- **`get_new_files()` no longer auto-advances the checkpoint**: `get_new_files()` now passes `save_checkpoint=False` to `get_directory_changes()` so the checkpoint is only written when the caller explicitly calls `save_checkpoint()`.
